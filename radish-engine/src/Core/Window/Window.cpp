...
namespace rds
{
	...
	void Window::Initialize(const WindowProperties& properties)
	{
		...
		if (!glfwWindow)
			RDS_LOG_CRITICAL("Failed to create a valid GLFW window. GLFW is probably uninitialized");
	...
	void Window::SetWindowCallbacks()
	{
		...
		glfwSetKeyCallback(m_Window.get(), [](GLFWwindow* window, int key, int scancode, int action, int mods) {
			...
			switch (action)
			{
				...
				default:
				{
					RDS_LOG_WARN("Invalid keyboard action passed to window callback: {}", action);
				} break;
			}
		});
